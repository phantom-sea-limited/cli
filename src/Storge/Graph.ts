import { defaultOA } from "./OnedriveOA";
import { join, basename } from "path";

interface ME extends JSON {
...
class defaultAPI {
    ...
    /** 远程路径,必须以/开头 */
    remote: string
    constructor(OA: defaultOA, remote: string = "/") {
        this.OA = OA
        this.remote = remote
    }
    ...
    async get(url: URL | string,) {
        // console.log(url)
        const JWT = await this.OA.get_token()
...
class GraphAPI extends defaultAPI {
    ...
    async thumbnail(id: string): Promise<thumbnail> {
        return this.get(`${this.BASEv1}me/drive/items/${id}/thumbnails?$select=large`)
    }

    async thumbnails(id: string): Promise<CHILDREN> {
        return this.get(`${this.BASEv1}me/drive/items/${id}/children?$expand=thumbnails&$select=thumbnails,id,name`)
    }

    async preview(id: string): Promise<preview> {
        return this.post(`${this.BASEv1}me/drive/items/${id}/preview`, null)
    }
    ...
    async u_creatUploadsession(path: string): Promise<createUploadSession> {
        const p = join(this.remote, path.replace(/[:|"|*|<|>|?|\\|\/|。]/g, ""))
        return this.post(`${this.BASEv1}me/drive/root:${p}:/createUploadSession`, JSON.stringify({
            "item": {
                "@microsoft.graph.conflictBehavior": "replace",
                "name": basename(p)
            }
        })) as Promise<createUploadSession>
    }
    ...
    async uploadChunks(uploadSessionUrl: string, data: Blob, chunkSize: number = 5 * 1024 * 1024) {
        const slices = this.sliceFile(data);
        let index = 0;
        async function uploadNextChunk() {
            ...
            const headers = {
                'Content-Range': `bytes ${chunkStart}-${chunkEnd}/${data.size}`,
    ...
    async Upload(name: string, data: any) {
        ...
        const url = (await this.u_creatUploadsession(name)).uploadUrl
        if (url) {
            // console.log(`文件${name}开始上传`)
            await this.uploadChunks(url, file)
        } else {
            console.error(`${name}创建上传会话失败`)
            throw new Error(`${name}创建上传会话失败`)
        }
    }
